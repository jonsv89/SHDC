...
if(as.numeric(args[1])==1){
  ...
  enoughsamples<-resultstab[unique(sort(c(femid,malid))),]
  enoughsamples<-cbind(gsub("_.+","",rownames(enoughsamples)),gsub(".+_","",rownames(enoughsamples)),enoughsamples)
  colnames(enoughsamples)<-c("disease","study id","women (cases)","women (controls)","men (cases)","men (controls)")
  write.table(enoughsamples,"Submission/Figures and Tables/Datasets.txt",quote=F,sep="\t",row.names = F)
