...
pathwaytables<-function(comparison,icdcode,experiment,filtro,colorcat,icdnombres){
  ...
  for(a in 1:length(files)){
    ...
    if(comparison!="Cases" && comparison!="Controls"){
      sexes<-c(sexes,paste(as.numeric(table(targets$Category)[c("Case","Control")])[1],"Case/",as.numeric(table(targets$Category)[c("Case","Control")])[2],"Control",sep=""))
    }
  ...
  for(a in 1:length(names(allpathlist))){
    ...
    if(comparison!="Cases" && comparison!="Controls"){
      colnames(fortheforest)[2]<-"differential_expression"
      fortheforest$differential_expression[which(as.numeric(fortheforest$mean)>0)]<-"up-regulated"
      fortheforest$differential_expression[which(as.numeric(fortheforest$mean)<0)]<-"down-regulated"
      ## Add the color to women and men lines ##
      collabels<-names(table(fortheforest$differential_expression))
      # collabels<-collabels[-which(collabels=="")]
      colvalues<-rep("#FFFFFF",length(collabels))
      ...
      p<-forestplot(
        ...
        estimate = mean,
        logodds = FALSE,
        colour=differential_expression,
        title = paste("ICD (",names(allpathlist)[a],")\n(",genesaltered[a]," -- ",sexes[a],")\n",as.character(icdnombres[names(allpathlist)[a]]),sep=""),
        xlab = thexlab
      )+
        ggplot2::scale_colour_manual(values=colvalues,labels=collabels)+
  ...
  for(a in 1:length(allcats)){
    ...
    if(comparison!="Cases" && comparison!="Controls"){
      colnames(fortheforestcat)[2]<-"differential_expression"
      ## Add the color to women and men lines ##
      collabels<-names(table(fortheforestcat$differential_expression))
      colvalues<-rep("#FFFFFF",length(collabels))
      if(length(which(collabels=="up-regulated"))>0){colvalues[which(collabels=="up-regulated")]<-"#C81E17"}
      if(length(which(collabels=="down-regulated"))>0){colvalues[which(collabels=="down-regulated")]<-"#3D4D8B"}
      thexlab<-"Case vs. Control comparison (NES)\n(positive values => up-regulated in cases)\n(negative values => down-regulated in cases)"
...
## Women vs. Men ##
cases9<-pathwaytables("Cases","ICD9","Microarrays","REACTOME",colorcat9,icd9nombres)
controls9<-pathwaytables("Controls","ICD9","Microarrays","REACTOME",colorcat9,icd9nombres)
women9<-pathwaytables("Women","ICD9","Microarrays","REACTOME",colorcat9,icd9nombres)
men9<-pathwaytables("Men","ICD9","Microarrays","REACTOME",colorcat9,icd9nombres)
adjusted9<-pathwaytables("Adjusted","ICD9","Microarrays","REACTOME",colorcat9,icd9nombres)
...
adjusted10<-pathwaytables("Adjusted","ICD10","Microarrays","REACTOME",colorcat10,icd10nombres)
